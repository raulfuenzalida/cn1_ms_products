CREATE TABLE IF NOT EXISTS products (
    ...
    CONSTRAINT chk_filament_grams_positive
        CHECK (filament_grams > 0),

    CONSTRAINT chk_printing_hours_positive
        CHECK (printing_hours > 0),

    CONSTRAINT chk_profit_percentage_range
        CHECK (profit_percentage >= 0),

    CONSTRAINT chk_product_status_valid
        CHECK (status IN ('ACTIVE', 'INACTIVE')),

    CONSTRAINT chk_price_status_valid
        CHECK (price_status IN ('CURRENT', 'OUTDATED')),

    INDEX idx_products_public_catalog (
        status,
        price_status,
        final_price
    ),

    INDEX idx_products_id_filament (
        id_filament
    )
...
CREATE TABLE IF NOT EXISTS product_images (
    ...
    CONSTRAINT chk_image_type_valid
        CHECK (image_type IN ('FRONT', 'SIDE', 'TOP')),

    INDEX idx_product_images_product_type (
        id_product,
        image_type,
        is_active
    )
